...
import com.steve1316.uma_android_automation.bot.DialogEventProducer
		...
		if (!isRunning) {
			...
			thread = thread {
				try {
					...
					dialogEventProducer = DialogEventProducer(game, coroutineScope, eventBus)
					...
					game.start("", coroutineScope, eventBus)
		...
		if (!isRunning) {
			...
			thread = thread {
				try {
					...
					// Start with the provided settings from SharedPreferences.
					game.start("Daily Tasks", coroutineScope, eventBus)
					...
					if (e.toString() == "java.lang.InterruptedException") {
						...
						MessageLog.log("$appName encountered an Exception: ${e.stackTraceToString()}", tag = tag, isError = true)
					}
				} finally {
					//performCleanUp()
				}
